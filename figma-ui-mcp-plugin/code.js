...
handlers.create = async (params) => {
  ...
  if (type === "FRAME" || type === "GROUP") {
    ...
    if (width) {
      node.resize(width, height || node.height);
    }
...
handlers.modify = async (params) => {
  ...
  // If node is TEXT, load font first before doing ANY property changes (e.g. resize, characters, etc.)
  if (node.type === "TEXT") {
    try {
      ...
      try { await figma.loadFontAsync(node.fontName); } catch (e2) {}
    }
  }
  ...
  if (node.type === "TEXT" && params.textAutoResize !== undefined) {
    node.textAutoResize = params.textAutoResize;
  }
...
handlers.setupDesignTokens = async function(params) {
  ...
  if (tsNames.length > 0) {
    ...
    if (Object.keys(existingTextStyles).length === 0 && false && figma.getLocalTextStyles) {
...
handlers.applyTextStyle = async function(params) {
  ...
  if (!resolvedStyleId) {
    ...
    catch (e) { if (false && figma.getLocalTextStyles) styles = figma.getLocalTextStyles(); }
