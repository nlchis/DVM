...
> Canonical pattern cho mọi BA skill (`/brainstorm`, `/explore`, `/urd`, `/brd`, `/prd`, `/srs`) chạy SAU Write doc, TRƯỚC khi suggest downstream skills. Mục đích: không để OQ debt accumulate cross-stage — buộc resolve hoặc ack hold ngay.
...
1. **Own OQs** — vừa write trong doc hiện tại (parse §"Open Questions" hoặc "Các câu hỏi mở").
2. **Inherited OQs** — từ upstream chain còn `status: hold` hoặc `[ ]`. Upstream chain tuỳ skill:

| Skill | Upstream chain để inherit OQs |
|---|---|
| `/explore` | (none — explore là gốc tìm hiểu) |
| `/brainstorm` | `docs/{feature}/explores/*.md` §"Các câu hỏi mở" (nếu có) |
| `/urd` | `docs/{feature}/brainstorms/*.md` hoặc `explores/*.md` §"Open Questions" / "Các câu hỏi mở" |
| `/brd` | explores + brainstorm + `urd.md` |
| `/prd` | explores + brainstorm + `urd.md` + `brd.md` |
| `/srs` | explores + brainstorm + `urd.md` + `brd.md` + `prd.md` |

Skill scan §"Open Questions" / "Các câu hỏi mở" của mỗi upstream doc, extract OQs còn `[ ]` (unresolved) hoặc `[~]` (deferred but not closed). Bỏ qua `[x]` (resolved).

## Step 2 — Prompt user

In danh sách OQs theo format tiếng Việt:
...
|---|---|
| `/explore` | `brainstorms/*.md`, `urd.md`, `brd.md`, `prd.md`, `srs/spec.md` (nếu tồn tại) |
